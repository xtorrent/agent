...
#ifndef OP_OPED_NOAH_TOOLS_BBTS_AGENT_CONSISTENT_HASH_RING_HPP
#define OP_OPED_NOAH_TOOLS_BBTS_AGENT_CONSISTENT_HASH_RING_HPP
...
inline static uint32_t hash_crc32(
        const char *key,
        size_t key_length,
...
public:
    ...
    void add_node(const Node &node, int32_t replicas) {
        ...
        for (int32_t r = 0; r < replicas; ++r) {
            ...
            uint32_t hash = hash_crc32(vnode.c_str(), vnode.size(), NULL);
    ...
    const Node& get_node(const std::string &data) const {
        ...
        uint32_t hash = hash_crc32(data.c_str(), data.size(), NULL);
...
#endif // OP_OPED_NOAH_TOOLS_BBTS_AGENT_CONSISTENT_HASH_RING_HPP
